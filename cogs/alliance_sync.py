...
class AllianceSync(commands.Cog):
    ...
    async def _notify_sync_unavailable(self, channel, interaction_message=None):
        """Alliance sync has no player-data source; tell the admin where to set kingdoms instead."""
        embed = discord.Embed(
            title=f"{theme.warnIcon} Alliance Sync Unavailable",
            description=(
                f"{theme.upperDivider}\n"
                f"Automatic member sync is turned off - member nicknames, levels and kingdoms "
                f"can no longer be refreshed automatically.\n\n"
                f"Set member kingdoms under **Alliance Management -> Member Kingdoms**.\n"
