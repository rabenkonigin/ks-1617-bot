...
NAMED_WEEKDAY = {
    "Viking Vengeance": 1,     # Tuesday
    "Swordland Showdown": 6,   # Sunday
    "Tri-Alliance Clash": 5,   # Saturday
    "Eternity's Reach": 1,     # Tuesday
...
def test_cycle_event_same_day_is_not_skipped():
    ...
    cfg = get_event_config("Swordland Showdown")  # global_biweekly, cycle 2
    ref = _reference(cfg)
    event_day = ref + timedelta(weeks=cfg["cycle_weeks"] * 5)
    asked = event_day.replace(hour=8, minute=0)
    nxt = calculate_next_occurrence("Swordland Showdown", asked)
...
def test_cycle_event_steps_one_cycle_after_event_day():
    ...
    cfg = get_event_config("Swordland Showdown")
    ref = _reference(cfg)
    event_day = ref + timedelta(weeks=cfg["cycle_weeks"] * 5)
...
def test_viking_vengeance_same_day_tuesday_kept():
    tue_ref, _ = calculate_viking_vengeance_dates(FROM)
    asked = tue_ref.replace(hour=6, minute=0)
    tue, thu = calculate_viking_vengeance_dates(asked)
    assert tue.date() == tue_ref.date(), "Viking Vengeance Tuesday must survive same-day setup"
    assert thu == tue + timedelta(days=2)


# --- Viking Vengeance two-day cycle ---

def test_viking_vengeance_tuesday_thursday_pairing():
    # KS advances the Tuesday and Thursday independently, so ask from a date
    # before the cycle's Tuesday to exercise a single-cycle pair (Mon 2026-01-12).
    frm = datetime(2026, 1, 12, 10, 30, tzinfo=pytz.UTC)
    tue, thu = calculate_viking_vengeance_dates(frm)
    ...
    ref = _reference(get_event_config("Viking Vengeance"))
