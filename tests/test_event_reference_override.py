...
def test_override_shifts_occurrence(tmp_path, monkeypatch):
    ...
    cfg = net.get_event_config("Swordland Showdown")
    default_next = net.calculate_next_occurrence("Swordland Showdown", FROM)

    # Shift the rotation by one week (still the same weekday).
    shifted_ref = (
        datetime.strptime(cfg["reference_date"], "%Y-%m-%d") + timedelta(weeks=1)
    ).strftime("%Y-%m-%d")
    net.set_reference_override(9, "Swordland Showdown", shifted_ref)

    overridden = net.calculate_next_occurrence("Swordland Showdown", FROM, guild_id=9)
    ...
    assert net.calculate_next_occurrence("Swordland Showdown", FROM, guild_id=8) == default_next
    assert net.calculate_next_occurrence("Swordland Showdown", FROM) == default_next
...
def test_clearing_override_restores_default(tmp_path, monkeypatch):
    ...
    net.set_reference_override(9, "Swordland Showdown", shifted_ref)
    assert net.get_reference_override(9, "Swordland Showdown") == shifted_ref

    net.set_reference_override(9, "Swordland Showdown", None)
    assert net.get_reference_override(9, "Swordland Showdown") is None
    assert net.calculate_next_occurrence("Swordland Showdown", FROM, guild_id=9) == \
        net.calculate_next_occurrence("Swordland Showdown", FROM)


def test_crazy_joe_respects_override(tmp_path, monkeypatch):
    _override_db(tmp_path, monkeypatch)
    cfg = net.get_event_config("Viking Vengeance")
    default_tue, _ = net.calculate_viking_vengeance_dates(FROM)
    shifted_ref = (
        datetime.strptime(cfg["reference_date"], "%Y-%m-%d") + timedelta(weeks=1)
    ).strftime("%Y-%m-%d")
    net.set_reference_override(9, "Viking Vengeance", shifted_ref)

    tue, thu = net.calculate_viking_vengeance_dates(FROM, guild_id=9)
